...
IntelliCam is a preset-driven camera app (React Native + Expo Router). Full technical design lives in [ARCHITECTURE.md](docs/ARCHITECTURE.md), current build status in [PROJECT_STATE.md](docs/PROJECT_STATE.md), and the editable system view in [INTELLICAM_PROJECT_OVERVIEW.drawio](docs/diagrams/INTELLICAM_PROJECT_OVERVIEW.drawio).
...
- Photos are saved through Expo MediaLibrary to the on-device `IntelliCam`
  ...
  planned for relational app data only and must never store image blobs. Its
  proposed structure is documented in
  [INTELLICAM_LOCAL_DATABASE.drawio](docs/diagrams/INTELLICAM_LOCAL_DATABASE.drawio).
